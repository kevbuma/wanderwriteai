...
if page == "Generate Guide":
    ...
    summary = st.text_area("Optional: Add a brief summary or highlight for the city")
    if st.button("Generate Guide") and city:
        with st.spinner("Generating guide..."):
            prompt = f"Write an engaging, adventure-style travel guide for the city: {city}."
            if summary:
                prompt += f" Here is a brief input from the user: {summary}"
            prompt += " Include local highlights, food, hidden gems, and unique cultural experiences."
            try:
                ...
                st.text_area("Your Travel Guide", guide, height=300, key=f"guide_{city}")

                if st.button("Add to Book", key=f"add_{city}"):
    ...
    if st.button("Generate Surprise Guide"):
        with st.spinner("Generating guide..."):
            ...
            try:
                ...
                if st.button("Add to Book", key=f"surprise_add_{random_city}"):
