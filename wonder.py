...
if "book_storage" not in st.session_state:
    st.session_state.book_storage = {}
    ...
    if option == "Register":
        ...
        if st.button("Login"):
            ...
            if user and user["password"] == hash_password(password):
                ...
                st.session_state.book_storage = user["book"]
...
if page == "Generate Guide":
    ...
    city_summary = st.text_area("Optional: Add a short summary or theme for the guide")
    if st.button("Generate Guide") and city:
        with st.spinner("Generating guide..."):
            ...
            if city_summary:
                prompt += f" Focus especially on this theme: {city_summary}"
            try:
                ...
                if st.button("Add to Book"):
                    st.session_state.book_storage.append({"city": city, "guide": guide})
                    st.session_state.users[st.session_state.username]["book"] = st.session_state.book_storage
    ...
    if st.button("Generate Surprise Guide"):
        with st.spinner("Generating guide..."):
            ...
            try:
                ...
                if st.button("Add to Book"):
                    st.session_state.book_storage.append({"city": random_city, "guide": guide})
                    st.session_state.users[st.session_state.username]["book"] = st.session_state.book_storage
                    st.success("Guide added to your book.")
            except Exception as e:
                st.error(f"Something went wrong: {e}")
...
if page == "My Book":
    st.header("📘 My Book")
    if not st.session_state.book_storage:
        st.info("No guides added yet.")
    else:
        for entry in st.session_state.book_storage:
        ...
        if st.button("Download My Book"):
            if export_format == "PDF":
                ...
                pdf_output.seek(0)
                st.download_button("Download PDF", data=pdf_output.read(), file_name="my_book.pdf", mime="application/pdf")
            else:
                txt_output = ""
                for entry in st.session_state.book_storage:
