    ...
    if option == "Register":
        ...
        if st.button("Login"):
            ...
            if user and user["password"] == hash_password(password):
                ...
                st.session_state.book = user["book"]
...
if page == "Home":
    ...
    st.subheader("📋 Sample Guide Template")
    st.markdown("""
...
if page == "Generate Guide":
    ...
    summary = st.text_area("Optional Brief Summary for the City")

    if st.button("Generate Guide") and city:
        with st.spinner("Generating guide..."):
            prompt = f"Write an engaging, adventure-style travel guide for the city: {city}. Include local highlights, food, hidden gems, and unique cultural experiences. {summary}"
            try:
                ...
                guide_formatted = f"# Travel Guide: {city}\n\n" + guide.replace("\n\n", "\n### ")
                st.markdown(guide_formatted)

                if st.button("Add to Book"):
                    st.session_state.book.append({"city": city, "guide": guide_formatted})
    ...
    if st.button("Generate Surprise Guide"):
        with st.spinner("Generating guide..."):
            ...
            try:
                ...
                guide = response.choices[0].message.content
                guide_formatted = f"# Travel Guide: {random_city}\n\n" + guide.replace("\n\n", "\n### ")
                st.markdown(guide_formatted)

                if st.button("Add to Book"):
                    st.session_state.book.append({"city": random_city, "guide": guide_formatted})
                    st.session_state.users[st.session_state.username]["book"] = st.session_state.book
                    st.success("Guide added to your book.")
            except Exception as e:
                st.error(f"Something went wrong: {e}")
