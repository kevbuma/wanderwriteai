...
# ----------- Logo Display -----------
logo_url = "https://i.imgur.com/yourlogo.png"  # Replace with actual logo URL or base64 if you have it
...
city = st.text_input("Enter a city name", value="")
facts = st.text_area("Paste some facts about this city (from RandomCity.net, Wikipedia, etc.)", value="")

if st.button("🎲 Surprise Me with a City"):
    selection = random.choice(random_cities)
    city = selection["city"]
    facts = selection["facts"]
    st.success(f"🎉 Surprise! Selected city: {city}")

# Refresh input fields with Surprise Me selection
city = st.text_input("Enter a city name", value=city)
facts = st.text_area("Paste some facts about this city (from RandomCity.net, Wikipedia, etc.)", value=facts)
