...
if st.button("🎲 Surprise Me with a City"):
    ...
    st.rerun()  # Refresh app with new inputs
...
# ----------- Display Guide and Add to Book -----------
if "current_guide" in st.session_state and st.session_state.current_guide:
    st.markdown("### ✨ Your Travel Guide")
    st.write(st.session_state.current_guide)

    if st.button("➕ Add this guide to my Book"):
        st.session_state.book_guides.append({
            "city": st.session_state.current_city,
            "guide": st.session_state.current_guide
        })
        st.success(f"Added '{st.session_state.current_city}' to your book!")

    # TXT download
    txt_file = io.BytesIO()
    txt_file.write(st.session_state.current_guide.encode('utf-8'))
    txt_file.seek(0)
        ...
        def add_text(self, text):
            for line in text.split('\n'):
                try:
                    ...
                    self.multi_cell(0, 10, "[Encoding Error]")

    pdf = PDF()
    pdf.add_text(st.session_state.current_guide)
...
if st.session_state.book_guides:
    ...
    if st.button("📥 Download Book as TXT"):
        ...
        book_file.seek(0)
        st.download_button("⬇️ Download TXT Book", data=book_file, file_name="WanderWrite_Book.txt", mime="text/plain")
    ...
    if st.button("📥 Download Book as PDF"):
        ...
        pdf_bytes = pdf.output(dest='S').encode('latin1', 'replace')
        pdf_file = io.BytesIO(pdf_bytes)
        st.download_button("⬇️ Download PDF Book", data=pdf_file, file_name="WanderWrite_Book.pdf", mime="application/pdf")
